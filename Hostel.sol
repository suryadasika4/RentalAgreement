...
contract Hostel{
    ...
    modifier onlyLandlord(uint _index) {
        ...
        require(msg.value >= uint(Room_by_No[_index].rent_per_month), "Not enough CCN in your wallet");
        _;
    }


        modifier enoughAgreementfee(uint _index) {
        require(msg.value >= uint(uint(Room_by_No[_index].rent_per_month) + uint(Room_by_No[_index].securityDeposit)), "Not enough CCN in your wallet");
        _;
    }
